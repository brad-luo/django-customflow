...
from .models import LogObjectRelation, StateObjectRelation, Transition, Workflow, WorkflowModelRelation, \
...
def set_log(obj, state, source_state=None, transition=None):
    LogObjectRelation.objects.create(content=obj, state=state, source_state=source_state, transition=transition)


def get_log(obj):
    ctype = ContentType.objects.get_for_model(obj)
    return LogObjectRelation.objects.filter(content_type=ctype, content_id=obj.id).all()
